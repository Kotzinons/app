...
export default function StorybookCover({ size = "md", className }) {
  ...
  // Use first 5 characters in display order
      ...
      >
        ...
        <svg className="absolute inset-0 w-full h-full" preserveAspectRatio="xMidYMid slice">
          {Array.from({ length: 35 }).map((_, i) => {
            ...
            const y = ((i * 113) % 600);
            const r = (i % 3) * 0.5 + 0.5;
            return <circle key={i} cx={x} cy={y} r={r} fill="#FFE08A" opacity={0.6} />;
          })}
        </svg>
        ...
        <div className="relative px-4 mt-3 text-center">
          <h3 className={cn("font-serif italic gold-text leading-none", s.title)} style={{ textShadow: "0 2px 20px rgba(245,192,51,0.4)" }}>
            Origins
          </h3>
        ...
        <div className="absolute bottom-4 left-0 right-0 flex justify-center gap-1.5 px-3">
          {heroes.map((h) => (
            <div
              ...
              style={{
                boxShadow: `0 4px 16px -4px ${h.color_hex}`,
              }}
            >
              <img
                src={h.image_url}
